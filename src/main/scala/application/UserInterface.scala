...
class UserInterface(userName: String) {
  ...
  final def userPickSquare(userXOrOChoice: String = "", numSquaresChosen: Int = 0): Square = {
    ...
    userSquareChoice match {
      case "9" => println("There are no squares left to pick")
        Square("", "")

      case _ => println("Please choose your next square")
        Square(userXOrOChoice, userSquareChoice)
        userPickSquare(userXOrOChoice, numSquaresChosen + 1)
    }
  }

  def drawSquare(userXOrOChoice: String): Unit = {
